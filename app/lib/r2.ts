...
export async function listR2Objects(prefix = "", includeAll = false): Promise<R2ObjectItem[]> {
  try {
    ...
    // 1. Cloudflare Workers native R2 binding list
    ...
    // 2. Pure REST fetch fallback for local environments
    const host = `${ACCOUNT_ID}.r2.cloudflarestorage.com`;
    ...
    if (res.ok) {
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        if (!includeAll && k.endsWith(".json") && !k.includes("contracts_index")) continue;
        items.push({
          key: k,
          size: sizes[i] || 0,
